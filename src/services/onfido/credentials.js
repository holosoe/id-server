...
function validateSession(session, sessionId) {
  if (!session) {
    return {
      ...
      log: `onfido/credentials: Failed to retrieve Verrif session ${sessionId}. Exiting.`,
    };
  }
  if (session.status !== "success") {
    return {
      error: `Verification failed. Status is '${session.status}'. Expected 'success'.`,
      log: `onfido/credentials: Verification failed. Status: ${session.status}. Exiting.`,
    };
  }
  if (session.verification?.code !== 9001) {
    return {
      error: `Verification failed. Verification code is ${session.verification?.code}. Expected 9001.`,
      log: `onfido/credentials: Verification failed. Verification code: ${session.verification?.code}. Exiting.`,
    };
  }
  if (session.verification.status !== "approved") {
    return {
      ...
      log: `onfido/credentials: Verification status is ${session.verification.status}. Exiting.`,
    };
  }
  ...
  for (const field of necessaryPersonFields) {
    if (!(field in person)) {
      return {
        ...
        log: `onfido/credentials: Verification missing necessary field: ${field}. Exiting.`,
      };
    }
  }
  ...
  //     log: `onfido/credentials: Verification missing necessary field: address. Exiting.`,
  //   };
  // }
  // if (!("postcode" in address)) {
  //   return {
  //     error: "Verification missing necessary field: postcode.",
  //     log: `onfido/credentials: Verification missing necessary field: postcode. Exiting.`,
  //   };
  // }
  ...
  if (!doc) {
    return {
      ...
      log: `onfido/credentials: Verification missing necessary field: document. Exiting.`,
    };
  }
  if (!("country" in doc)) {
    return {
      error: "Verification missing necessary field: country.",
      log: `onfido/credentials: Verification missing necessary field: country. Exiting.`,
    };
  }
  return { success: true };
}
...
async function getCredentials(req, res) {
  try {
    ...
    const session = await getOnfidoCheck(check_id);

    const validationResult = validateSession(session, check_id);
    ...
    if (user) {
      await saveCollisionMetadata(uuid, check_id, session);
    ...
    await redactVeriffSession(check_id);

import { UserCredentials, UserProofMetadata } from "../init";
...
async function getProofMetadata(req: Request, res: Response) {
	...
	try {
		...
		console.log(err);
		console.log(
			"GET /proof-metadata: Could not find user proof metadata. Exiting",
		);
...
async function postProofMetadata(req: Request, res: Response) {
	...
	if (typeof sigDigest !== "string") {
		logWithTimestamp(
			"POST /proof-metadata: sigDigest isn't a string. Exiting.",
		);
		return res.status(400).json({ error: "sigDigest isn't a string" });
	}
	...
	try {
		...
		console.log(err);
		console.log(
			"POST /proof-metadata: Could not save userProofMetadataDoc. Exiting",
		);
		return res.status(400).json({

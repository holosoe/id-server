...
async function createSession(_: Request, res: Response) {
	...
	try {
		console.log(env.VERIFF_PUBLIC_API_KEY);
		const config = {
			headers: {
				"Content-Type": "application/json",
				"X-AUTH-CLIENT": env.VERIFF_PUBLIC_API_KEY,
			},
		};
		...
		logWithTimestamp(
			`POST veriff/session: Created session ${verification?.id}`,
		);
		return res
			.status(200)
			.json({ url: verification?.url, id: verification?.id });
	} catch (err) {

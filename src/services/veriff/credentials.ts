...
const veriffPublicKey = env.VERIFF_PUBLIC_API_KEY;
const veriffSecretKey = env.VERIFF_SECRET_API_KEY;
...
function extractCreds(session: {
	verification: {
		...
		document: { country: keyof typeof countryCodeToPrime };
	};
}) {
	const person = session.verification.person;
	const address = person.addresses?.[0]?.parsedAddress;
	assert.ok(
		session.verification.document.country in countryCodeToPrime,
		"Unsupported country",
	);
...
async function getVeriffSessionDecision(sessionId: $TSFixMe) {
	try {
		...
		console.error(`Error getting session with ID ${sessionId}`, `${err}`!);
		return {};
	}
}

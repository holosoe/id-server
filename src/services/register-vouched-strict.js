/**
 * Contains endpoint for "strict" issuer that uses Vouched.
 * "Strict" here means that the issuer blocks users who are
 * on sanctions lists and other lists.
 */
...
function validateJob(job, jobID) {
  ...
  // Assert user is not on sanctions list
  if (
    job.result.aml?.data?.matchStatus === "potential_match" ||
    job.result.aml?.data?.matchStatus === "true_positive"
  ) {
    logWithTimestamp(
      `registerVouched/vouchedCredentials: user is on sanctions list. Exiting.`
    );
    return { error: "User is on sanctions list" };
  }
  if (job.result.aml?.riskLevel === "medium" || job.result.aml?.riskLevel === "high") {
    logWithTimestamp(
      `registerVouched/vouchedCredentials: user is on sanctions list. Exiting.`
    );
    return { error: "User is on sanctions list" };
  }
  if (job.result.aml?.totalHits > 0 || job.result.aml?.totalMatches > 0) {
    logWithTimestamp(
      `registerVouched/vouchedCredentials: user is on sanctions list. Exiting.`
    );
    return { error: "User is on sanctions list" };
  }
  if (job.result.aml?.hits?.politicalPositions?.length > 0) {
    logWithTimestamp(
      `registerVouched/vouchedCredentials: user is on sanctions list. Exiting.`
    );
    return { error: "User is on sanctions list" };
  }
  if (job.result.aml?.hits?.matches?.sanction?.length > 0) {
    logWithTimestamp(
      `registerVouched/vouchedCredentials: user is on sanctions list. Exiting.`
    );
    return { error: "User is on sanctions list" };
...
function extractCreds(job) {
  // TODO: Extract sanctions risk level !!!!
...
async function getCredentials(req, res) {
  ...
  if (process.env.ENVIRONMENT == "dev") {
    ...
    logWithTimestamp("registerVouched/vouchedCredentials (v2): Generating signature");

    const response = issue(
      process.env.STRICT_ISSUER_PRIVKEY,
  ...
  if (!req?.query?.jobID) {
    logWithTimestamp(
      "registerVouched/vouchedCredentials (v2): No job specified. Exiting."
    );
  ...
  if (user) {
    logWithTimestamp(
      `registerVouched/vouchedCredentials (v2): User has already registered. Exiting. UUID == ${uuid}`
    );
    return res
      .status(400)
  ...
  logWithTimestamp(
    `registerVouched/vouchedCredentials (v2): Inserting user into database`
  );
  ...
  logWithTimestamp("registerVouched/vouchedCredentials (v2): Issuing credentials");
  ...
  logWithTimestamp(
    `registerVouched/vouchedCredentials (v2): Returning user whose UUID is ${uuid}`
  );
